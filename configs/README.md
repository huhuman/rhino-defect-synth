...
| `spalling.rebar.enabled`, `spalling.rebar.place_probability`, `spalling.rebar.*` | mixed | Rebar placement and geometry controls. | `component_defect_defaults.yaml` | All visible candidates are evaluated, then each candidate is independently kept with `place_probability`. |
| `spalling.rebar.radius_choices_cm`, `spalling.rebar.spacing_choices_cm`, `spalling.rebar.cover_depth_range`, `spalling.rebar.max_aggregate_size_cm` | `list[float]`, `list[float]`, `list[float,float]`, `float` | Discrete bridge-style bar radius/spacing presets, sampled cover depth, and aggregate-size input for minimum spacing. | `component_defect_defaults.yaml` | Minimum center spacing follows an AASHTO-style clear-spacing proxy before visibility filtering. |
| `spalling.rebar.visible_length_min_cm`, `spalling.rebar.visible_length_min_diameter_factor`, `spalling.rebar.curve_*`, `spalling.rebar.rib_*` | mixed | Visibility gating plus centerline/rib detail for exposed rebars. | `component_defect_defaults.yaml` | Candidate bars must remain visible inside the sampled spall depth before geometry is created. |
