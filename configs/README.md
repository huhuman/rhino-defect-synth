...
| `reference.*` | mixed | Candidate point extraction controls. | `component_defect_defaults.yaml` | Includes `sample_edge_length_u/v`, `boundary_margin_ratio_u/v`, and legacy `sample_count_u/v` fallback; `boundary_margin_ratio_*` is used both for sampling trim and candidate validation, and `trim_margin` is no longer used. |
...
| `spalling.rebar_enabled`, `spalling.rebar_probability`, `spalling.force_rebar`, `spalling.rebar.*` | mixed | Rebar placement and geometry controls. | `component_defect_defaults.yaml` | Exposed-rebar geometry is still recorded as one defect, but layers/materials are split into `defect::exposed_rebar::CS*::spalling` and `defect::exposed_rebar::CS*::rebar`. |
| `spalling.rebar.rebar_radius_choices_cm`, `spalling.rebar.rebar_spacing_choices_cm`, `spalling.rebar.rebar_cover_depth_range` | `list[float]`, `list[float]`, `list[float,float]` | Discrete bridge-style bar radius/spacing presets and sampled cover depth. | `component_defect_defaults.yaml` | Defaults are in centimeters and are sampled before visibility filtering. |
| `spalling.rebar.rebar_visible_length_min_cm`, `spalling.rebar.rebar_visible_length_min_diameter_factor`, `spalling.rebar.rebar_curve_*`, `spalling.rebar.rebar_rib_*` | mixed | Visibility gating plus centerline/rib detail for exposed rebars. | `component_defect_defaults.yaml` | Candidate bars must remain visible inside the sampled spall depth before geometry is created. |
