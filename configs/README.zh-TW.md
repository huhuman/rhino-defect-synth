...
| `preparation.autosave.disable_during_batch` | `bool` | 在長時間的 `*_batch.py` 執行期間，暫時停用 Rhino autosave。 | `true` | 會在 `finally` 還原；若 Rhino FileSettings API 不可用則忽略。 |
| `preparation.undo.disable_during_batch` | `bool` | 在長時間的 `*_batch.py` 執行期間，暫時停用 Rhino undo recording。 | `true` | 會在 `finally` 還原；與週期性 undo 清理是不同層級的保護。 |
| `modeling` | `dict` | 傳入 `pipeline.create_model`。 | config | 需包含 `strategy`。 |
| `rendering` | `dict` | 傳入 `pipeline.run_render`。 | config | render stage 必要。 |
| `nested_loop` | `dict` | 由 `main_cube_batch.run` 與 `main_component_batch.run` 使用，用於批次資料產生。 | 無 | 可選；`main.py` 不會使用。 |

## Batch 迴圈參數（`nested_loop`，主要給 `main_cube_batch.py`；其中穩定化子集也供 `main_component_batch.py` 使用）
...
| `spalling.rebar.enabled`, `spalling.rebar.place_probability`, `spalling.rebar.*` | mixed | rebar 放置與幾何控制。 | `component_defect_defaults.yaml` | 會先列出所有可見 candidate，再用 `place_probability` 逐根隨機決定是否建立。 |
| `spalling.rebar.radius_choices_cm`, `spalling.rebar.spacing_choices_cm`, `spalling.rebar.cover_depth_range`, `spalling.rebar.max_aggregate_size_cm` | `list[float]`, `list[float]`, `list[float,float]`, `float` | 橋梁常見 bar 半徑/spacing 離散選項、保護層深度抽樣，以及最小間距計算用的骨材尺寸。 | `component_defect_defaults.yaml` | 會先套用 AASHTO 風格的最小 clear-spacing 代理，再做可見性篩選。 |
| `spalling.rebar.visible_length_min_cm`, `spalling.rebar.visible_length_min_diameter_factor`, `spalling.rebar.curve_*`, `spalling.rebar.rib_*` | mixed | exposed rebar 的可見性門檻與曲折/竹節細節控制。 | `component_defect_defaults.yaml` | 只有在抽樣 spall depth 下仍可見的 candidate 才會真的建立幾何。 |
...
| `log_output_timings` | `bool` | 是否記錄每個 frame 的輸出分段耗時。 | `false` | 可用來判斷 slowdown 是卡在 color、buffer、depth/normal 或 mask。 |
